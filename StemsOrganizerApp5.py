...
import io
APP_VERSION = "1.1.0" # <-- ADICIONE ESTA LINHA
...
class ExecutionFeedback:
    ...
    def update_stats(self, stat_type, increment=1):
        ...
        try:
            if "📁 Processados" in self.stat_labels and self.stat_labels["📁 Processados"].winfo_exists():
                self.stat_labels["📁 Processados"].configure(text=str(self.stats['processed']))
            if "🤖 Classificados" in self.stat_labels and self.stat_labels["🤖 Classificados"].winfo_exists():
                self.stat_labels["🤖 Classificados"].configure(text=str(self.stats['classified']))
            if "🗑️ Descartados" in self.stat_labels and self.stat_labels["🗑️ Descartados"].winfo_exists():
                self.stat_labels["🗑️ Descartados"].configure(text=str(self.stats['discarded']))
            if "⏱️ Tempo" in self.stat_labels and self.stat_labels["⏱️ Tempo"].winfo_exists():
                elapsed = int(time.time() - self.stats['start_time'])
                mins, secs = divmod(elapsed, 60)
                self.stat_labels["⏱️ Tempo"].configure(text=f"{mins:02d}:{secs:02d}")
        except:
            pass
...
class App:
    def __init__(self, root):
        ...
        self.root.minsize(900, 600)  # FIX: Tamanho mínimo

        ctk.set_appearance_mode("Dark")
        self.root.configure(fg_color=COLOR_BACKGROUND)

        # FIX: Inicializar todas as variáveis antes de usar
    ...
    def init_supabase(self):
        ...
        try:
            ...
            # Teste a conexão de forma mais leve
    ...
    def create_widgets(self):
        ...
        # FIX: Aplicar botão inicialmente oculto
...
"""
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, "r", encoding='utf-8') as f:
                    key = f.read().strip()
                    if key and len(key) > 20:  # FIX: Validação mais rigorosa
    ...
    def save_api_key(self, key, popup):
        ...
        key = key.strip()
        if not key or len(key) < 20:  # FIX: Validação mais rigorosa
    ...
    def open_settings_window(self):
        ...
        version_label = ctk.CTkLabel(
            about_frame,
            text="Versão 1.3 - Bug Fix Edition",
            font=("", 12),
            text_color="#888888"
        )
...
© 2024 Prod. Aki - Todos os direitos reservados"""
    ...
    def test_api_key(self, key):
        ...
        try:
            ...
            model = genai.GenerativeModel("gemini-1.5-flash")  # FIX: Usar modelo mais estável
    ...
    def show_final_report(self):
        ...
        stats = [
            ("📁 Arquivos a Mover", moved, COLOR_ACCENT_PURPLE),
            ("🗑️ Arquivos a Deletar", deleted, COLOR_ERROR),
            ("✏️ Arquivos a Renomear", renamed, COLOR_WARNING),
            ("📊 Total de Operações", total, COLOR_ACCENT_CYAN)
        ]
        ...
        for action in self.planned_actions:
            if action[0] == 'move':
                ...
                categories["Descartados"].append((filename, "🗑️"))
            elif action[0] == 'rename':
                src, dest = action[1:]
                filename = os.path.basename(dest)
                if "Renomeados" not in categories:
                    categories["Renomeados"] = []
                categories["Renomeados"].append((filename, "✏️"))
        ...
        for category, files in sorted(categories.items()):
            ...
            header = ctk.CTkLabel(
                category_frame,
                text=f"📂 {category} ({len(files)} arquivos)",
                font=("", 16, "bold"),
                text_color=COLOR_ACCENT_CYAN,
                anchor="w"
            )
            ...
            for filename, icon in files[:10]:  # Mostrar até 10 arquivos por categoria
    ...
    def start_apply_thread(self):
        """Inicia aplicação das mudanças"""
        if self.is_processing:
            messagebox.showwarning("Aviso", "Uma operação já está em andamento.")
            return
            
        if not self.planned_actions:
            messagebox.showwarning("Aviso", "Nenhuma ação para aplicar.")
            return
        ...
        confirm_msg = f"Confirma a aplicação de {total_actions} operações?\n\n"
        confirm_msg += f"• {moved} arquivos serão movidos\n"
        confirm_msg += f"• {deleted} arquivos serão deletados\n\n"
        confirm_msg += "Esta operação não pode ser desfeita!"
        
        if not messagebox.askyesno("Confirmar Operações", confirm_msg):
            return
    ...
    def apply_organization_logic(self):
        """Aplica as mudanças com feedback visual melhorado"""
        total_actions = len(self.planned_actions)
        if total_actions == 0:
            return
            
        # Inicializar feedback para aplicação
        for widget in self.visual_organizer_frame.winfo_children():
            widget.destroy()
        ...
        try:
            ...
            if fail_count == 0:
                self.update_status("Todas as alterações aplicadas com sucesso!", 1.0)
                status_msg = "✅ Operação Concluída com Sucesso!"
                status_color = COLOR_SUCCESS
            else:
                self.update_status(f"Operação concluída com {fail_count} falhas.", 1.0)
                status_msg = f"⚠️ Concluída com {fail_count} Falhas"
                status_color = COLOR_WARNING
            
            # Mostrar resultado final
            self.root.after(0, lambda: self.show_apply_results(successful_actions, failed_actions, status_msg, status_color))
            self.root.after(100, lambda: self.play_lightning_effect(self.apply_button if self.apply_button.winfo_exists() else self.start_button))
    ...
    def show_apply_results(self, successful_actions, failed_actions, status_msg, status_color):
        """Mostra resultados da aplicação"""
        for widget in self.visual_organizer_frame.winfo_children():
            widget.destroy()
        ...
        # Cards de estatísticas
        success_card = ctk.CTkFrame(stats_inner, fg_color=COLOR_SUCCESS)
        success_card.grid(row=0, column=0, padx=10, pady=5, sticky="ew")
        ctk.CTkLabel(success_card, text="✅ Sucessos", font=("", 12, "bold")).pack(pady=(10, 5))
        ctk.CTkLabel(success_card, text=str(len(successful_actions)), font=("", 20, "bold")).pack(pady=(0, 10))
        
        if failed_actions:
            fail_card = ctk.CTkFrame(stats_inner, fg_color=COLOR_ERROR)
            fail_card.grid(row=0, column=1, padx=10, pady=5, sticky="ew")
            ctk.CTkLabel(fail_card, text="❌ Falhas", font=("", 12, "bold")).pack(pady=(10, 5))
            ctk.CTkLabel(fail_card, text=str(len(failed_actions)), font=("", 20, "bold")).pack(pady=(0, 10))
        
        total_card = ctk.CTkFrame(stats_inner, fg_color=COLOR_ACCENT_PURPLE)
        total_card.grid(row=0, column=2, padx=10, pady=5, sticky="ew")
        ctk.CTkLabel(total_card, text="📊 Total", font=("", 12, "bold")).pack(pady=(10, 5))
        ...
        if failed_actions:
            ...
            for action, error in failed_actions[:20]:  # Mostrar até 20 falhas
                ...
                ctk.CTkLabel(fail_frame, text="❌", width=30).pack(side="left", padx=5)
                ctk.CTkLabel(fail_frame, text=desc, anchor="w").pack(side="left", fill="x", expand=True, padx=5, pady=5)
    ...
    def reset_for_new_analysis(self):
        """Reseta a interface para nova análise"""
        self.planned_actions = []
        self.folder_path_full = ""
        self.folder_path_entry.configure(state="normal")
        self.folder_path_entry.delete(0, ctk.END)
        ...
        self.start_button.configure(state="disabled")
        self.show_welcome_screen()
        self.update_status("Pronto para nova análise.", 0)

    def retry_failed_actions(self, failed_actions):
        """Tenta novamente as ações que falharam"""
    ...
    def descobrir_prefixo_recorrente(self, nomes_de_arquivos):
        """Descobre prefixo recorrente com melhor detecção"""
        if not nomes_de_arquivos:
            return None
            
        # Padrões mais específicos para detecção de prefixos
        patterns = [
            r'^([A-Za-z0-9]+_)',  # Letras/números seguidos de underscore
            r'^([^a-zA-Z0-9\s]+)',  # Símbolos no início
            r'^(\d+[\-_\.])',  # Números seguidos de separador
        ]
        ...
        for pattern in patterns:
            ...
            for nome in nomes_de_arquivos:
                match = prefix_pattern.match(nome.strip())
                if match:
            ...
            if prefixes:
                ...
                if most_common and most_common[0][1] >= MIN_PREFIX_OCCURRENCES:
                    all_prefixes.append((most_common[0][0], most_common[0][1]))
        
        # Retornar o prefixo mais comum entre todos os padrões
        if all_prefixes:
            return max(all_prefixes, key=lambda x: x[1])[0]
        
        return None

    def armazenar_acao(self, action_type, *params):
        """Armazena ação com validação"""
        if action_type in ['move', 'delete', 'rename']:
            self.planned_actions.append((action_type, *params))
    ...
    def mover_arquivo(self, caminho_original, nome_final, categoria, pasta_raiz, is_dry_run):
        ...
        if is_dry_run:
            ...
            try:
                os.makedirs(dest_folder, exist_ok=True)
                shutil.move(caminho_original, os.path.join(dest_folder, nome_final))
                return True
            except Exception as e:
    ...
    def renomear_arquivo_no_local(self, caminho_original, nome_final, is_dry_run):
        """Renomeia arquivo com validação melhorada"""
        ...
        # Evitar renomeação desnecessária
    ...
    def is_audio_insignificant(self, audio_path, avg_threshold=-55, peak_threshold=-50, deep_check=False):
        """Verifica se áudio é insignificante com melhor detecção"""
        try:
            ...
            # Análise básica - só volume médio
            avg_db = audio.dBFS
            if avg_db < avg_threshold:
                if not deep_check:
                    return True
                
                # Análise profunda - verificar picos também
                max_db = audio.max_dBFS
                if max_db < peak_threshold:
                    return True
                    
                # Verificar se há silêncio prolongado (mais que 80% do arquivo)
                silent_ranges = audio.detect_silence(min_silence_len=100, silence_thresh=avg_threshold)
                if silent_ranges:
            ...
            print(f"DEBUG: Erro ao analisar áudio {audio_path}: {e}")
            return False
    ...
    def classificar_com_ia_mestre(self, nomes_arquivos):
        ...
        for attempt in range(max_retries):
            try:
                # Usar modelo mais recente com configuração otimizada
                generation_config = {
                ...
                if not match:
                    raise json.JSONDecodeError("Nenhum JSON encontrado na resposta", response.text, 0)
                
                resultados = json.loads(match.group(0))
                
                # Validar categorias
                valid_categories = set(valid_categories_list.replace(" ", "").split(','))
                for nome, categoria in resultados.items():
                    if categoria not in valid_categories:
                        resultados[nome] = "Outros"
                
                return resultados
                
            except Exception as e:
                print(f"DEBUG: Tentativa {attempt + 1} falhou: {e}")
    ...
    def submit_suggestion_to_supabase(self, nome_arquivo, categoria):
        """Submete sugestão ao Supabase com melhor tratamento de erro"""
        ...
        try:
            ...
            # Buscar por palavras-chave mais inteligentemente
            keywords = re.findall(r'\b[a-zA-Z]{3,}\b', clean_name.lower())
            if not keywords:
                return
            
            # Usar a palavra mais significativa (geralmente a primeira ou mais longa)
            keyword = max(keywords, key=len) if len(keywords) > 1 else keywords[0]
            ...
            print(f"DEBUG: Falha ao enviar sugestão para '{nome_arquivo}': {e}")
